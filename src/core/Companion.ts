...
class CompanionDriver {
  ...
  send(
    ...
    this.#buttons.forEach(async (b) => {
      try {
        await fetch(
          `http://${this.host}:${this.port}/api/custom-variable/${b.variableName}/value`,
          {
            method: "POST",
      ...
      try {
        await fetch(
          `http://${this.host}:${this.port}/api/location/${b.page}/${b.row}/${b.col}/press`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
          },
...
const companionPlugin: FastifyPluginAsync<CompanionDriverOptions> = async (
  fastify,
  opts,
) => {
