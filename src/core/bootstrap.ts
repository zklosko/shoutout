...
export async function bootstrap(): Promise<BootstrapConfig> {
    let settings = await db.select().from(settingsTable).where(eq(settingsTable.id, "settings")).get()
    if (!settings) {
        console.log("Could not retreive settings table from db. Creating new one.")
        const inserted = await db.insert(settingsTable)
            .values({ id: "settings", infoText: "", approverPassword: "changeme", port: 8080 })
            .onConflictDoNothing()
            .returning()
            .get()
    ...
    if (!connections) {
        ...
        const inserted = await db.insert(connectionsTable).values({
            type: "companion",
            host: "127.0.0.1",
            port: 8000
        })
