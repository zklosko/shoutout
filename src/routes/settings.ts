...
export async function settingsRoutes(fastify: FastifyInstance, options: {}) {
  ...
  (fastify.get("/", async (request, response) => {
    ...
    const connections = await db
      ...
      .where(eq(connectionsTable.type, "companion"))
      .get();
    const buttons = await db.select().from(companionButtonsTable).all();

    if (!settings || !connections || !buttons)
      throw Error(
      ...
      async (request, response) => {
        ...
        try {
          ...
          await db
            ...
            .set({ host: companionHost, port: companionPort })
            .where(eq(connectionsTable.type, "companion"))
            .returning()
            .get();
          ...
          const buttonsQuery = Object.keys(buttonsUpdate).length
            ? await db
                .update(companionButtonsTable)
                .set(buttonsUpdate)
                .where(eq(companionButtonsTable.id, 1))
                .returning()
                .get()
            : await db
                .select()
                .from(companionButtonsTable)
                .where(eq(companionButtonsTable.id, 1))
                .get();
